...
public class LoopingStatefulTimer extends DoFn<KV<String, TableRow>, TableRow> {
    ...
    private final StateSpec<ValueState<Long>> loopingTimerTime = StateSpecs.value(BigEndianLongCoder.of());
    ...
    public void process(
        ...
        @TimerId("loopingTimer") Timer loopingTimer) {
            ...
            Long currentTimeValue = loopingTimerTime.read();
            Instant nextTimerTimeBasedOnCurrentElement = c.timestamp().plus(Duration.standardSeconds(20));
    ...
    public void onTimer(
        ...
        @TimerId("loopingTimer") Timer loopingTimer) {
            ...
            String row_key = key.read();
            Instant timestamp = c.timestamp();
            String ts = timestamp.toString();

            TableRow new_row = new TableRow()
                .set("device_id", row_key)
                .set("timestamp", ts)
                .set("value", null)
            ...
            Instant nextTimer = c.timestamp().plus(Duration.standardSeconds(20));
