...
public class IoTStreamBigQuery {
  ...
  public static void main(String[] args) throws IOException {
    ...
    TimeSeriesOptions options = 
      PipelineOptionsFactory.fromArgs(args).as(TimeSeriesOptions.class);
    ...
    messages =
        pipeline.apply(
            "Pull PubSub Messages", PubsubIO.readStrings().fromTopic(options.getInputTopic())).apply("Convert PubSub messages to TableRow Type", new JsonToTableRow());

    PCollection<TSDataPoint> stream = messages.apply(
      "convert to TS Data Point", ParDo.of(new ParseTSDataPointFromPubSub()));
